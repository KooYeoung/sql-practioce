...
-- ============================================================================
--  HR 스키마 (PostgreSQL)  —  파트 06 토픽 6-1 'SQL 이해하기' 공용 실습 데이터
-- ----------------------------------------------------------------------------
--  Oracle 고전 HR 샘플 스키마(7테이블)의 PostgreSQL 이식본 — *전체 데이터* 포함
--  (regions 4 · countries 25 · locations 23 · jobs 19 · departments 27 ·
--   employees 107 · job_history 10).
--
--  출처: https://gist.github.com/gourab5139014/b0b8e90c66acd5d0e9bcfebbff65d02a
--        (Oracle HR 스키마를 PostgreSQL 로 이식한 공개 학습 자료)
--  검증: PostgreSQL 16 적재 확인(오류 0). DBeaver/psql 모두 호환(순수 SQL).
--
--  사용법: 빈 데이터베이스에 그대로 실행 → 7개 테이블 생성 + 데이터 적재.
--    psql:    psql -d hr -f hr_schema_postgres.sql
--    DBeaver: SQL 에디터에 붙여넣고 전체 실행(스크립트 실행)
--
--  참고: 스크립트 끝에 인덱스(CREATE INDEX)와 일부 제약(CHECK/UNIQUE)이 포함돼 있다.
--        인덱스·제약 설계는 뒤 토픽(데이터베이스 설계)에서 다루므로, 여기서는
--        "이미 만들어져 있다" 정도로만 알고 넘어간다.
--  ※ 순환참조(departments.manager_id ↔ employees)는 원본 스크립트가
--    FK 추가 → 제거 → 데이터 적재 → FK 재추가 순서로 안전하게 처리한다.
-- ============================================================================

-- 다시 실행해도 안전하도록 기존 테이블을 먼저 정리한다(처음이면 그냥 통과).
DROP TABLE IF EXISTS job_history, employees, departments, jobs, locations, countries, regions CASCADE;
